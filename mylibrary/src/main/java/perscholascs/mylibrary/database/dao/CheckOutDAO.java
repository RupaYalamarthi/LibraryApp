...
public interface CheckOutDAO extends JpaRepository<Checkout,Long> {
  ...
  //  Select checkout.*, books.id FROM checkout inner JOIN transactions on transactions.checkout_id = checkout.id inner join
    //books on books.id = transactions.book_id group by books.id having checkout.user_id=5
